...
class TestCreateJob:
    ...
    def test_creates_job_with_predictable_id(self, job_store):
        """Job is created with ID from id_generator."""
        result = job_store.create_job("https://example.com/playlist")
    ...
    def test_first_job_should_start_immediately(self, job_store):
    ...
    def test_second_job_should_queue(self, job_store):
        """Second job should queue while first is active."""
        job_store.create_job("https://example.com/first")
        result = job_store.create_job("https://example.com/second")
    ...
    def test_custom_audio_format(self, job_store):
    ...
    def test_returns_none_when_queue_full(self, fixed_time, id_generator):
        ...
        result = store.create_job("https://example.com/3")
        assert result is None

    def test_prunes_finished_jobs_when_at_capacity(self, fixed_time, id_generator):
        """Finished jobs are pruned to make room for new ones."""
        store = JobStore(clock=lambda: fixed_time, id_generator=id_generator)
        store.MAX_JOBS = 2

        # Create and complete first job
        result1 = store.create_job("https://example.com/1")
        assert result1 is not None
        job1, _ = result1
        store.update_job(job1.id, status=JobStatus.COMPLETED)

        # Create second job
        store.create_job("https://example.com/2")

        # Third job should succeed (first job was pruned)
        result3 = store.create_job("https://example.com/3")
...
class TestGetJob:
    ...
    def test_returns_job_by_id(self, job_store):
        """Can retrieve a job by its ID."""
        result = job_store.create_job("https://example.com/playlist")
        assert result is not None
        created_job, _ = result

        retrieved = job_store.get_job(created_job.id)
        assert retrieved is not None
        assert retrieved.id == created_job.id
        assert retrieved.url == "https://example.com/playlist"

    def test_returns_none_for_nonexistent_job(self, job_store):
...
class TestCancelJob:
    ...
    def test_cancels_pending_job(self, job_store, fixed_time):
        """Can cancel a pending job."""
        result = job_store.create_job("https://example.com/playlist")
        assert result is not None
        job, _ = result

        success = job_store.cancel_job(job.id)
    ...
    def test_cannot_cancel_finished_job(self, job_store):
        """Cannot cancel an already finished job."""
        result = job_store.create_job("https://example.com/playlist")
        assert result is not None
        job, _ = result

        job_store.update_job(job.id, status=JobStatus.COMPLETED)
        success = job_store.cancel_job(job.id)

        assert success is False

    def test_cancel_nonexistent_job_returns_false(self, job_store):
        """Cancelling nonexistent job returns False."""
        success = job_store.cancel_job("nonexistent-id")
        assert success is False
...
class TestUpdateJob:
    ...
    def test_updates_status(self, job_store):
        """Can update job status."""
        result = job_store.create_job("https://example.com/playlist")
        assert result is not None
        job, _ = result

        updated = job_store.update_job(job.id, status=JobStatus.DOWNLOADING)
        assert updated is not None
        assert updated.status == JobStatus.DOWNLOADING

    def test_updates_progress(self, job_store):
        """Can update job progress."""
        result = job_store.create_job("https://example.com/playlist")
        assert result is not None
        job, _ = result

        updated = job_store.update_job(job.id, progress=50.0)
        assert updated is not None
        assert updated.progress == 50.0

    def test_updates_album_info(self, job_store):
        ...
        updated = job_store.update_job(job.id, album_info=album)

        assert updated is not None
        assert updated.album_info is not None
        assert updated.album_info.title == "Test Album"

    def test_finishing_job_sets_completed_at(self, job_store, fixed_time):
        """Finishing a job sets completed_at if not already set."""
        result = job_store.create_job("https://example.com/playlist")
        assert result is not None
        job, _ = result

        updated = job_store.update_job(job.id, status=JobStatus.COMPLETED)
        assert updated is not None
...
class TestAddLog:
    ...
    def test_adds_log_entry(self, job_store, fixed_time):
        """Can add log entries to a job."""
        result = job_store.create_job("https://example.com/playlist")
        assert result is not None
        job, _ = result

        job_store.add_log(job.id, "downloading", "Track 1: 50%")
    ...
    def test_trims_logs_when_exceeding_limit(self, fixed_time, id_generator):
        """Logs are trimmed when exceeding per-job limit."""
        store = JobStore(clock=lambda: fixed_time, id_generator=id_generator)
        store.MAX_LOGS_PER_JOB = 3

        result = store.create_job("https://example.com/playlist")
        assert result is not None
        job, _ = result

        # Add 5 logs (exceeds limit of 3)
        for i in range(5):
            store.add_log(job.id, "downloading", f"Log {i}")
...
class TestPopNextPending:
    ...
    def test_returns_oldest_pending_job(self, job_store):
        """Returns the oldest pending job (FIFO)."""
        job_store.create_job("https://example.com/1")  # Active
        job_store.create_job("https://example.com/2")  # Pending
        job_store.create_job("https://example.com/3")  # Pending

        next_job = job_store.pop_next_pending()
        assert next_job is not None
        assert next_job.id == "job-1"  # Second job (first pending)

    def test_returns_none_when_no_pending(self, job_store):
        """Returns None when no pending jobs."""
        job_store.create_job("https://example.com/1")  # Active only
...
class TestDeleteJob:
    ...
    def test_deletes_finished_job(self, job_store):
        ...
        job_store.update_job(job.id, status=JobStatus.COMPLETED)
        success = job_store.delete_job(job.id)

        assert success is True
        assert job_store.get_job(job.id) is None

    def test_cannot_delete_running_job(self, job_store):
...
class TestClearCompleted:
    ...
    def test_clears_finished_jobs(self, job_store):
        """Clears all finished jobs."""
        # Create and complete 2 jobs
        result1 = job_store.create_job("https://example.com/1")
        assert result1 is not None
        job1, _ = result1
        job_store.update_job(job1.id, status=JobStatus.COMPLETED)

        result2 = job_store.create_job("https://example.com/2")
        assert result2 is not None
        job2, _ = result2
        job_store.update_job(job2.id, status=JobStatus.FAILED)

        # Create pending job
        job_store.create_job("https://example.com/3")
...
class TestTimeout:
    ...
    def test_job_times_out_after_threshold(self, id_generator):
    ...
    def test_job_does_not_timeout_before_threshold(self, id_generator):
        ...
        still_running = store.get_job(job.id)
